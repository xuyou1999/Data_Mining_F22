...
for my_line_id in ["1","5","95","71","25","8","72","7"]:
    data_1218 = open(test_path, "r")
    df = pd.read_csv(train_path)
    # line = data_1218.readline()
    # line = data_1218.readline()
    # df['trip_id'] = df['trip_id'].apply(lambda x :hash(x))
    ...
    with data_1218 as f:
        ...
        for line in pbar:
            ...
            if not line_id == my_line_id: continue
            ...
            pbar.set_description("line:"+str(my_line_id)+mode+",avgerror: "+str(float('%.2f' % np.average(error_list)))+", avg_Q5 error: "+str(float('%.2f' % np.average(error_list_time))) +", rate: "+str([float('%.3f' % (x/sum(condition_record))) for x in condition_record]))

    error_file = open("result/error_list_"+str(my_line_id)+mode+".txt","w")
    ...
    error_file_time = open("result/error_list_"+str(my_line_id)+mode+"_time.txt","w")
