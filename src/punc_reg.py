...
def calc(trips, calendar, stop_times, actural_time):
    ...
    output_df = pd.DataFrame(columns=['org_row', 'route_id', 'direction_id', 'date', 'stop_id', 'on_time_rate', 'schedule_waiting_time', 'actual_waiting_time', 'excess_waiting_time', 'schedule_count', 'a_devided_s'])
    count = 0
    file_number = 0
    ...
    # for j in range(0, len(err_file)):
        ...
        try:
            ...
            output_df.loc[len(output_df)] =[org_row, route_id, direction_id, date, stop, on_time_rate, regularity_list['schedule_waiting_time'], regularity_list['actual_waiting_time'], regularity_list['excess_waiting_time'], schedule_count, a_devided_s]
            count += 1
            print(count)
        except:
